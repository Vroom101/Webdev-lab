...
if (isset($_GET["fullName"])) {
    ...
    $rationNumber = htmlspecialchars(trim($_GET["rationNumber"] ?? ''));
    $familySize = htmlspecialchars(trim($_GET["familySize"] ?? ''));
    $category = htmlspecialchars(trim($_GET["category"] ?? ''));
    $hobby = htmlspecialchars(trim($_GET["hobby"] ?? ''));
    ...
    $errors[] = "No direct script access allowed. Please submit the form via GET method.";
}
...
<head>
    ...
    <title>GET Registration Result</title>
...
<body>
    ...
    <div class="container-card">
        <div class="header-section" style="margin-bottom: 1rem;">
            <h2>Processor Mode <span class="badge">$_GET</span></h2>
        </div>
